...
-- Docker Entrypoint Initialization Script: Schema & Performance Indexes
-- Location: docker-entrypoint-initdb.d/schema.sql
-- ==============================================================================
-- This script ensures that 'transactions' and 'recentActivities' (and related)
-- tables are created and explicitly indexed upon MySQL container initialization.
-- It is idempotent: safe to execute on fresh databases or existing volumes.
-- ==============================================================================
...
-- ------------------------------------------------------------------------------
-- 1. TRANSACTIONS TABLE & INLINE INDICES
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 2. RECENT ACTIVITIES TABLE & INLINE INDICES
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 3. AUDIT LOGS TABLE & INLINE INDICES (System Telemetry & Historical Audits)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. IDEMPOTENT PROCEDURAL INDEX CREATION
-- Ensures missing indices are added even if tables already existed prior to run
-- ------------------------------------------------------------------------------
DELIMITER //
...
-- Apply explicit performance indices for 'transactions'
...
-- Apply explicit performance indices for 'recentActivities'
...
-- Apply explicit performance indices for 'audit_logs'
...
-- Clean up temporary helper procedure
DROP PROCEDURE IF EXISTS `AddIndexIfNotExists`;

-- Flush permissions and verify
FLUSH PRIVILEGES;
